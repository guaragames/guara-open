...
func _ready():
	...
	print(AmpliandoGlobal.reset)
	
	if AmpliandoGlobal.reset == true:
		seleciona_global()
	else:
...
func limpa_global():
	...
	AmpliandoGlobal.ultima_escolhida = []
	AmpliandoGlobal.ultimo_tracado = []
	AmpliandoGlobal.reset = false
	
func seleciona_global():
	var indice = AmpliandoGlobal.ultima_escolhida
...
func selecionar_imagem_aleatoria() -> void:
	...
	AmpliandoGlobal.ultima_escolhida = indice
...
func _on_button_button_down() -> void:
	...
	print(AmpliandoGlobal.ultima_escolhida)
	pegou.play()
	var pontuacao = verificar_proximidade_escalas()
	match pontuacao:
		3:
			AmpliandoGlobal.estrelas = 3
			await get_tree().create_timer(0.2).timeout
			get_tree().change_scene_to_file("res://ampliando/scenes/game_over.tscn")
		2:
			AmpliandoGlobal.estrelas = 2
			await get_tree().create_timer(0.2).timeout
			get_tree().change_scene_to_file("res://ampliando/scenes/game_over.tscn")
		1:
			AmpliandoGlobal.estrelas = 1
			await get_tree().create_timer(0.2).timeout
			get_tree().change_scene_to_file("res://ampliando/scenes/game_over.tscn")
		0:
			AmpliandoGlobal.estrelas = 0
			await get_tree().create_timer(0.2).timeout
			get_tree().change_scene_to_file("res://ampliando/scenes/game_over.tscn")
